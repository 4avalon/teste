document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayQuestion(index) {
        ...
        // Esconde o botão "Mostrar Resposta" inicialmente
    ...
    function selectOption(optionElement, correct, questionIndex, optionIndex) {
        ...
        // Mostra o botão "Mostrar Resposta" após selecionar uma opção
    ...
    function showFinalResult() {
        const totalQuestions = questions.length;
        let resultHTML = '<div>Quiz Completo!</div>';
        resultHTML += '<div>Você acertou ' + correctAnswersCount + ' de ' + totalQuestions + ' perguntas.</div>';

        resultHTML += '<div class="result-grid">';
        for (let i = 0; i < totalQuestions; i++) {
            const question = questions[i];
            const correctOption = Array.from(question.querySelectorAll('options option')).findIndex(opt => opt.getAttribute('correct') === 'true');
            const userOption = userAnswers[i];
            const isCorrect = userOption === correctOption;
            const answerDisplay = String.fromCharCode(65 + (userOption !== undefined ? userOption : -1));
            const correctAnswerDisplay = String.fromCharCode(65 + correctOption);
            const colorClass = isCorrect ? 'correct-answer' : 'incorrect-answer';

            resultHTML += '<div class="' + colorClass + '">';
            resultHTML += (i + 1) + '. ' + answerDisplay;
            if (!isCorrect && userOption !== undefined) {
                resultHTML += ' (' + correctAnswerDisplay + ')';
            }
            resultHTML += '</div>';
